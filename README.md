...
- **延伸語法**：定義清單（`詞\n: 解釋`）、高亮 `==重點==`、上標 `x^2^`／下標 `H~2~O`、Emoji 短碼 `:rocket:`（建置期轉為 Unicode 字元）、縮寫 `*[ABBR]:`、插入 `++ins++`、自訂容器 `:::name`
...
| `nord`              | 深色                  | nord                        | dark           |
| `xai`               | 跟隨系統淺/深自動切換 | 自動切換                    | 依系統偏好選色 |
| `xai-light`         | 淺色                  | tokyo-night-light           | default        |
| `xai-dark`          | 深色                  | tokyo-night-dark            | dark           |
